...
#function for replicate file to another server when file changes happen in server A
def replicatetoserver(filename):
	...
	proxy1 = xmlrpc.client.ServerProxy("http://localhost:12002/",allow_none=True)
	reply_from_1=proxy1.replicate_rpcb(filename,text,file_version_map[filename])
...
#function for make changes in own file when changes made by another server
def replicate(filename,text,file_version):
			...
			return "replication is successfull at A server"

#handling a read write req
...
def checkversion(RW,filename,msg):
			...
			return file_version
#register a method 
server.register_function(notcheckversion, "notcheckversion_rpc")
server.register_function(checkversion,"checkversion_rpc")
server.register_function(replicate,"replicate_rpca")
server.register_function(updated_file,"updated_file_rpc")
print ('SERVER_A SERVICE is ready to receive...')
